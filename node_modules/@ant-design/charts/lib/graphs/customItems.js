"use strict";
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.registerNodes = void 0;
var g6_1 = __importDefault(require("@antv/g6"));
var contants_1 = require("./contants");
var utils_1 = require("./utils");
var registerNodes = function () {
    g6_1.default.registerNode('card', {
        draw: function (cfg, group) {
            var _a;
            if (cfg === void 0) { cfg = {}; }
            var size = cfg.size || [100, 30];
            var height = 0; // 统计容器总高度，动态设置
            if (typeof size === 'number')
                size = [size, size];
            var style = __assign({ radius: 2, fill: '#fff' }, cfg.style);
            var color = style.stroke || cfg.color || '#5B8FF9';
            var radius = style.radius;
            // node box
            var shape = group.addShape('rect', {
                attrs: __assign({ x: 0, y: 0, width: size[0], height: size[1], stroke: color }, style),
                name: 'main-box',
                draggable: true,
            });
            // node title
            var titleTextShape;
            if (cfg.title) {
                var _b = utils_1.getContentAndStyle(cfg.title), text = _b.text, _c = _b.style, titleStyle = _c === void 0 ? cfg.titleStyle : _c;
                titleTextShape = group.addShape('text', {
                    attrs: __assign(__assign({ textBaseline: 'top', x: contants_1.Margin, y: 2, text: text }, contants_1.defaultTitleLabelCfg), utils_1.getConfig(titleStyle, group)),
                    name: 'title',
                });
            }
            var titleBox = titleTextShape ? titleTextShape.getBBox() : { height: size[1] / 2 };
            // title rect
            var titleRectShape = group.addShape('rect', {
                attrs: {
                    x: 0,
                    y: 0,
                    width: size[0],
                    height: titleBox.height + 4,
                    fill: color,
                    radius: [radius, radius, 0, 0],
                },
                name: 'title-rect',
                draggable: true,
            });
            titleTextShape === null || titleTextShape === void 0 ? void 0 : titleTextShape.toFront();
            // collapse marker
            var markerShape;
            if (cfg === null || cfg === void 0 ? void 0 : cfg.children) {
                markerShape = group.addShape('marker', {
                    attrs: {
                        x: size[0] / 2,
                        y: 0,
                        r: 6,
                        cursor: 'pointer',
                        symbol: cfg.collapsed ? g6_1.default.Marker.expand : g6_1.default.Marker.collapse,
                        stroke: color,
                        lineWidth: 1,
                        fill: '#fff',
                    },
                    name: 'collapse-icon',
                });
            }
            var titleRectBoxHeight = titleRectShape.getBBox().height;
            height += titleRectBoxHeight;
            // body
            var bodyShape;
            if (cfg.body) {
                var _d = utils_1.getContentAndStyle(cfg.body), text = _d.text, _e = _d.style, bodyStyle = _e === void 0 ? cfg.bodyStyle : _e;
                bodyShape = group.addShape('text', {
                    attrs: __assign(__assign({ textBaseline: 'top', x: contants_1.Margin, y: height + contants_1.Margin, text: text }, contants_1.defaultLabelCfg), utils_1.getConfig(bodyStyle, group)),
                    name: "body",
                });
                height += bodyShape.getBBox().height;
            }
            // footer
            var footerTextShape;
            if (cfg.footer) {
                if (bodyShape) {
                    height += contants_1.Margin;
                }
                var _f = utils_1.getContentAndStyle(cfg.footer), labelText = _f.text, _g = _f.style, labelStyle = _g === void 0 ? cfg.footerStyle : _g;
                footerTextShape = group.addShape('text', {
                    attrs: __assign(__assign({ textBaseline: 'top', x: contants_1.Margin, y: height + contants_1.Margin, text: labelText }, contants_1.defaultLabelCfg), utils_1.getConfig(labelStyle, group)),
                    name: "footer-label",
                });
                var _h = footerTextShape.getBBox(), width = _h.width, contentHeight = _h.height;
                var footerValueHeight = 0;
                if (typeof cfg.footer === 'object' && ((_a = cfg.footer) === null || _a === void 0 ? void 0 : _a.value)) {
                    var _j = utils_1.getContentAndStyle({
                        content: cfg.footer.value,
                        style: cfg.footer.valueStyle,
                    }), valueText = _j.text, _k = _j.style, valueStyle = _k === void 0 ? cfg.footerValueStyle || cfg.footerStyle : _k;
                    var valueTextShape = group.addShape('text', {
                        attrs: __assign(__assign({ textBaseline: 'top', x: width + contants_1.Margin * 2, y: height + contants_1.Margin, text: valueText }, contants_1.defaultLabelCfg), utils_1.getConfig(valueStyle, group)),
                        name: "footer-value",
                    });
                    var valueHeight = valueTextShape.getBBox().height;
                    footerValueHeight = valueHeight;
                }
                height += Math.max(contentHeight, footerValueHeight);
            }
            // 调整容器宽高
            if (bodyShape) {
                var desTextShapeBBox = bodyShape.getBBox();
                var width = size[0] > desTextShapeBBox.width + 16 ? size[0] : desTextShapeBBox.width + 16;
                shape.attr({ width: width, height: height + 16 });
                titleRectShape === null || titleRectShape === void 0 ? void 0 : titleRectShape.attr('width', width);
                markerShape === null || markerShape === void 0 ? void 0 : markerShape.attr({
                    x: width,
                    y: (height + 16) / 2,
                });
            }
            return shape;
        },
        update: undefined,
    }, 'single-node');
};
exports.registerNodes = registerNodes;
