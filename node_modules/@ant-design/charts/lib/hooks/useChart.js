...
var createNode_1 = __importDefault(require("../util/createNode"));
function useInit(ChartClass, config) {
    var chart = react_1.useRef();
    var chartOptions = react_1.useRef();
    ...
    var reactDomToString = function (source, path, type) {
        var hasPath = util_2.utils.hasPath, setPath = util_2.utils.setPath;
        ...
        setPath(source, path, function () {
            ...
            var statisticDom = util_2.utils.isType(statisticCustomHtml, 'Function')
                ? statisticCustomHtml.apply(void 0, arg) : statisticCustomHtml;
            if (util_2.utils.isType(statisticDom, 'String') ||
                util_2.utils.isType(statisticDom, 'Number') ||
                util_2.utils.isType(statisticDom, 'HTMLDivElement')) {
                return statisticDom;
            }
            return createNode_1.default(statisticDom, type);
        });
    };
    var processConfig = function () {
        var hasPath = util_2.utils.hasPath;
        // statistic
    ...
    react_1.useEffect(function () {
        if (chart.current && !util_1.isEqual(chartOptions.current, config)) {
            ...
            if (chartOptions.current) {
                ...
                changeData = util_1.isEqual(currentConfig, inputConfig);
            }
            if (changeData) {
            ...
            chartOptions.current = util_2.utils.deepClone(config);
        }
    }, [config]);
    react_1.useEffect(function () {
        ...
        chart.current = util_2.utils.clone(chartInstance);
        if (onReady) {
